...
typedef void(WINAPI *WRAISEPT_)(const wchar_t *, const char *);
...
int LoadFunctions()
{
  ...
  if (ptruutf8_ == 0)
    return 1;
  ptruansi_ = (RAISEPT_)GetProcAddress(FLibHandle, "ptruansi");
  if (ptruansi_ == 0)
	  return 1;
...
void RuAnsi(char *result, const wchar_t *source)
{
  ...
  char* s2 = (char*)malloc(3 * wcslen(source) + 1);
  if (s != 0 && s2 != 0)
  {
    if (!vsutfmode)
